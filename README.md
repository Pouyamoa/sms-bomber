...
> **لطفا توجه کنید**
> > حالا اسکریپت اجرا می‌شود و از شما `شماره موبایل شخص` را میخواهد،
...
> **لطفا توجه کنید**
> در داخل دستور بالا شماره‌ موبایلی که نوشته شده را با `شماره شخص مورد نظر` خود جایگزین کنید، بدون صفر در ابتدای آن.
> > عدد `1000` نوشته شده در آخر دستور بیانگر تعداد پیامک ارسالی به شماره موبایل مورد نظر می‌باشد، آن عدد را نسبت به نیاز خود تغییر دهید، اگر خواستید روی شماره موبایل خود تست کنید آنرا به حداقل تعداد یعنی 10 عدد کاهش دهید.
...
>>> اگر ترمینال شما قادر به کپی کردن فایل توسط دستور کلون نیست، آن را [از اینجا]([y](https://github.com/Pouyamoa/sms-bomber/blob/main/sms.py) دانلود کنید
>>> و سپس به جای دستور `cd sms-bomber`، از دستور `cd downloads` برای قرار گرفتن در محل ذخیره فایل استفاده کنید،
زیرا محل ذخیره شدن ابزار خود اکنون در پوشه دانلود حافظه دستگاه می‌باشد.
